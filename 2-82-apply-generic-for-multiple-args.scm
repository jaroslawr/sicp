...
(define (apply-generic op . args)
  (define (prepare-call type-tags)
    (define (attempt-coercion procs args result)
      (cond ((null? args) result)
            ...
            (else
             (attempt-coercion
              (cdr procs)
              (cdr args)
              (append result (list ((car procs) (car args))))))))
    (define (coerce coercion-type-tags)
      (if (null? coercion-type-tags)
          '()
          (let*
              ((coercion-type-tag (car coercion-type-tags))
               (coercion-procs
                (map (lambda (type-tag) (get-coercion type-tag coercion-type-tag)) type-tags))
               (coerced-args (attempt-coercion coercion-procs args '()))
               (coerced-type-tags (map type-tag coerced-args)))
            (if coerced-type-tags
                (let
                    ((proc (get op coerced-type-tags)))
                  (if proc
                      (cons proc coerced-args)
                      (coerce (cdr coercion-type-tags))))
                (coerce (cdr coercion-type-tags))))))
    (let ((proc (get op type-tags)))
      (if proc
          (cons proc args)
          (coerce type-tags))))
  (let* ((call (prepare-call (map type-tag args))))
    (if (not (null? call))
        (apply (car call) (map contents (cdr call)))
        (error "No suitable method found for this name and set of arguments"))))
