...
(define (and->if exp)
  (define (and->if-rec expressions)
    (let ((expression (car expressions)))
      (make-if (make-eq? expression 'false)
               'false
               (if (null? (cdr expressions))
                   'true
                   (and->if-rec (cdr expressions))))))
  (and->if-rec (cdr exp)))

(define (or->if exp)
  (define (or->if-rec expressions)
    (let ((expression (car expressions)))
      (make-if (make-eq? expression 'true)
               'true
               (if (null? (cdr expressions))
                   'false
                   (or->if-rec (cdr expressions))))))
  (or->if-rec (cdr exp)))

(define (make-eq? lvalue rvalue)
  (list 'eq? lvalue rvalue))
