...
(define (eval exp env)
  (cond ((and (list? exp) (not (null? exp)) (get 'eval (car exp)))
...
(put 'eval 'quote  (lambda (exp env) (eval-quotation exp env)))
...
(define (self-evaluating? exp)
  (cond ((number? exp) true)
        ((string? exp) true)
        ((and (list? exp) (null? exp)) exp)
        (else false)))



(define (eval-quotation exp env)
  (define (quotation-expression subexps)
    (if (null? subexps)
        '()
        `(cons ,(car subexps)
               ,(quotation-expression (cdr subexps)))))
  (let ((quoted (cadr exp)))
    (if (list? quoted)
        (eval (quotation-expression quoted) env)
        quoted)))
...
(define primitive-procedures
  (list (list 'eager-cons cons)
        (list 'eager-car car)
        (list 'eager-cdr cdr)
        (list 'eager-pair? pair?)
        (list 'null? null?)
        (list '+ +)
        (list '- -)
        (list '* *)
...
(define (lazy-cons? object)
  (and (pair? object) (eq? (car object) 'lazy-cons)))

(define (print-lazy-cons object)
  (let ((input (read-line)))
    (if (not (equal? input "q"))
        (if (lazy-cons? object)
            (begin
              (display (lazy-car object))
              (print-lazy-cons (lazy-cdr object)))
            (display object))
        false)))
...
(define (user-print object env)
  (cond
   ((lazy-cons? object)
    (apply (cdr object)
           (list '(lambda (p q)
                    (define (traverse lazy-cons)
                      (if (not (equal? (read-line) "q"))
                          (if (eager-pair? lazy-cons)
                              ((eager-cdr lazy-cons)
                               (lambda (j k)
                                 (display j)
                                 (newline)
...
;; (define (car z)
;;   ((eager-cdr z) (lambda (p q) p)))

;; (define (cdr z)
;;   ((eager-cdr z) (lambda (p q) q)))
