...
;;            pairs))
;;
;; It is not clear from the book how exactly prime-sum-pair operates -
;; if it generates all the possible pairs in order, then I would
;; expect this code to return:
;;
;; ((3 20) (3 110) (8 35))
...
(define (analyze-if-fail exp)
  (let ((test-proc (analyze (if-predicate exp)))
        (fail-proc (analyze (if-consequent exp))))
    (lambda (env succeed fail)
      (test-proc env
                 (lambda (test-value fail2)
                   (succeed test-value fail2))
                 (lambda ()
