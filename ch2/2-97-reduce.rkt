...
(define (install-polynomial-package)
  ...
  (define (terms-accumulate terms mapfn initial combinator)
    (define (iter terms result)
      (if (empty-termlist? terms)
          result
    ...
    (define (convert-term term)
      ...
      (contents
       (terms-accumulate
        (term-list poly)
        convert-term
        (make-polynomial new-var (make-sparse-terms (list)))
        (lambda (a b)
  ...
  (define (pseudoremainder-terms terms1 terms2)
    (let* ((order1 (order (first-term terms1)))
           (order2 (order (first-term terms2)))
           (lead (coeff (first-term terms2)))
           (factor (exp lead (- (+ 1 order1) order2))))
      (cdr
       (div-terms
        (mul-terms terms1 (make-sparse-terms (list (list factor 0))))
        terms2))))
  (define (gcd-terms a b)
    (if (empty-termlist? b)
        (let ((terms-gcd
               (apply gcd
                      (terms-accumulate a (lambda (term) (coeff term))
                                        '() (lambda (result coeff) (cons coeff result))))))
        (terms-map a (lambda (term) (make-term (/ (coeff term) terms-gcd) (order term)))))
        (gcd-terms b (pseudoremainder-terms a b))))
  (define (gcd-poly p1 p2)
    (if (same-variable? (variable p1) (variable p2))
        (make-poly (variable p1) (gcd-terms (term-list p1) (term-list p2)))
        (error "Polynomials contain different variables -- GCD-POLY" (list p1 p2))))
  (define (reduce-terms terms1 terms2)
    (let* ((gcd (gcd-terms terms1 terms2))
           (order1 (max (order (first-term terms1)) (order (first-term terms2))))
           (order2 (order (first-term gcd)))
           (lead (coeff (first-term gcd)))
           (factor (exp lead (- (+ 1 order1) order2)))
           (factor-terms (make-sparse-terms (list (list factor 0)))))
      (cons
       (car (div-terms (mul-terms factor-terms terms1) gcd))
       (car (div-terms (mul-terms factor-terms terms2) gcd)))))
  (define (reduce-poly p1 p2)
    (if (same-variable? (variable p1) (variable p2))
        (let ((terms (reduce-terms (term-list p1) (term-list p2))))
          (cons (tag (make-poly (variable p1) (car terms)))
                (tag (make-poly (variable p1) (cdr terms)))))
    (error "Polynomials contain different variables -- REDUCE-POLY" (list p1 p2))))
  (define (tag p)
