...
(put 'eval 'make-unbound! (lambda (exp env) (eval-make-unbound exp env)))
...
;;; To make it symmetic to the antagonistic "(define x y)" I decided
;;; to only undefine variables from the current frame - you cannot
;;; define a variable in a frame other then the current one, so to be
;;; consistent you also should not be able to undefine one.

(define (undefine-variable! var env)
  (define (skip-var-loop vars vals)
    (if (not (null? (cdr vars)))
        (if (eq? (cadr vars) var)
            (set-cdr! vars (cddr vars))
            (skip-var-loop (cdr vars) (cdr vals)))
        (error "This variable isn't bound anyway -- UNDEFINE-VARIABLE!")))
  (let* ((frame (first-frame env))
         (vars (frame-variables frame))
         (vals (frame-values frame)))
    (if (eq? (car vars) var)
        (set-car! frame (cdr frame))
        (skip-var-loop vars vals))))
