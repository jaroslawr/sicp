     ...
     (list
      (make-if condition
               (make-begin
                (append
                 (for-body exp)
                 (list (cons '__for_loop (list increment)))))
               'false)))))

(define (for-declarations exp)
  (cadr exp))

(define (for-binding exp)
  (car (for-declarations exp)))

(define (for-condition exp)
  (cadr (for-declarations exp)))

(define (for-increment exp)
  (caddr (for-declarations exp)))
...
(define (make-frame variables values)
  (define (construct-bindings variables values)
    (cond ((and (null? variables) (null? values))
           '())
           ((not (and (null? variables) (null? values)))
            (cons (cons (car variables) (car values))
                  (construct-bindings (cdr variables) (cdr values))))
           (else
            (error "The number of variables isn't equal to the number of values -- MAKE-FRAME"))))
  (cons 'frame (construct-bindings variables values)))
...
(define (add-binding-to-frame! var val frame)
  (let ((bindings (frame-bindings frame))
    ...
    (if bindings
        (set-cdr! frame (cons binding bindings))
        (set-cdr! frame (list binding)))))
...
(define (lookup-variable-value var env)
  (cond ((eq? env the-empty-environment)
        ...
        ((frame-lookup (first-frame env) var) => cdr)
        (else
...
(define (set-variable-value! var val env)
  (if (eq? env the-empty-environment)
      ...
      (let* ((frame (first-frame env))
             (binding (frame-lookup frame var)))
        (if binding
            (set-cdr! binding val)
